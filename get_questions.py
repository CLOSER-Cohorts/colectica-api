...
def main():
    ...
    df_instrument_set, instrument_info = C.item_info_set('uk.cls.nextsteps', Id_instrument)

    with open(os.path.join(outdir, 'instrument.txt'), 'w') as f:
    ...
    df_question_all.to_csv(os.path.join(outdir, 'question.csv'), index=False, sep=';')

    df_codelist_all = pd.concat(codelist_df_list)
    df_codelist_all.to_csv(os.path.join(outdir, 'codelist.csv'), index=False, sep=';')

    df_response_all = pd.concat(response_df_list)
    df_response_all.to_csv(os.path.join(outdir, 'response.csv'), index=False, sep=';')
